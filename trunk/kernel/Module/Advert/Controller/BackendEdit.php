...
class Module_Advert_Controller_BackendEdit extends Module_Advert_Controller_BackendCommon
{
    ...
    public function run()
    {
        ...
        {
            $this->advert = self::getMapper('Advert/Advert')->createNew();
        }

        $this->user = self::getMapper('User/User')->findById($this->advert->getIdUser());
        ...
        $this->getView()->tree = self::getMapper('Category/Category')->findCategoriesByActive(1);
    ...
    {
        $this->advert = self::getMapper('Advert/Advert')->createFromCover($this->getRequest()->getPost('advert'),
                                                              array('category', 'id_user', 'type', 'header',
                                                                    'text', 'price', 'user_name', 'main_user_name',
                                                                    'phone', 'main_phone', 'email', 'main_email',
                                                                    'url', 'main_url', 'icq', 'main_icq',
        ...
        else
        {
            ...
            self::getMapper('Advert/Advert')->save($this->advert);

            $category = self::getMapper('Category/Category')->findById(
