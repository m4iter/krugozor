...
class Module_Advert_Controller_View extends Module_Common_Controller_Common
{
    public function run()
    {
        ...
        $advert_data = self::getMapper('Advert/Advert')->findByIdUrl(
        ...
        $path_to_category = self::getMapper('Category/Category')->loadPath($this->getView()->category->getId());
        ...
        // похожие объ€влени€
        $this->getView()->similar_adverts = self::getMapper('Advert/Advert')->finfSimilarAdverts($advert_data['advert'], $advert_data['user']);
        ...
        else
        {
            ...
            {
                self::getMapper('Advert/Advert')->incrementViewCount($advert_data['advert']);
            }
        }
