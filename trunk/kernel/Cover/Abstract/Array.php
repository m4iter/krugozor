 ...
 * в рамках OO-ориентированных PHP-приложений. Основной целью при создании данного класса
 * были следующие причины и проблемы при проектировании:
 *
 * 1. В PHP невозможно получать значение массива, если массив возвращается функцией или методом:
 * $object->foo()['key'];
 * данный класс позволяет это делать:
 * $object->foo()->key
 * или даже $object->foo()->key->subkey
 *
 * 2. Была необходима возможность получать NULL в случае отсутствующего элемента массива:
 * $object->non_exists_prop; // возвращает NULL
 *
 * 3. Была необходима возможность создавать коллекции объектов.
 *
 * В тоже время нужно было сохранить возможность работать с объектом как с массивом через
 * его стандартный интерфейс, что было обеспечено интерфейсом SPL ArrayAccess.
 */
abstract class Cover_Abstract_Array extends Cover_Abstract_Simple implements IteratorAggregate, Countable, ArrayAccess
{
